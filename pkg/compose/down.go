...
func (s *composeService) stopContainer(ctx context.Context, service *types.ServiceConfig, ctr containerType.Summary, timeout *time.Duration) error {
...
func (s *composeService) stopContainers(ctx context.Context, serv *types.ServiceConfig, containers []containerType.Summary, timeout *time.Duration) error {
	eg, ctx := errgroup.WithContext(ctx)
	for _, ctr := range containers {
		eg.Go(func() error {
			// return s.stopContainer(ctx, w, serv, ctr, timeout)
			return s.stopContainer(ctx, serv, ctr, timeout)
		})
	}
	return eg.Wait()
}
...
func (s *composeService) stopAndRemoveContainer(ctx context.Context, ctr containerType.Summary, service *types.ServiceConfig, timeout *time.Duration, volumes bool) error {
	...
	err := s.stopContainer(ctx, service, ctr, timeout)
	if cerrdefs.IsNotFound(err) {
		// w.Event(progress.RemovedEvent(eventName))
		return nil
	}
